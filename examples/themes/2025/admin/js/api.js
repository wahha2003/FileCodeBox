...
async function apiRequest(url, options = {}) {
    // 动态获取当前的authToken（优先检查内存变量，再检查 localStorage 的统一键 user_token）
    const currentAuthToken = window.authToken || localStorage.getItem('user_token');
...
function isAuthenticated() {
    const currentAuthToken = (typeof window !== 'undefined' ? window.authToken : null) || localStorage.getItem('user_token');

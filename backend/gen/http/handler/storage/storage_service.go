...
func GetStorageInfo(ctx context.Context, c *app.RequestContext) {
	...
	for key, value := range info.StorageDetails {
		storageDetails[key] = &storage.StorageDetail{
			Type:         value.Type,
	...
	c.JSON(consts.StatusOK, &storage.GetStorageInfoResp{
		Code:    200,
		Message: "获取成功",
...
func UpdateStorageConfig(ctx context.Context, c *app.RequestContext) {
	...
	c.JSON(consts.StatusOK, &storage.UpdateStorageConfigResp{
		Code:    200,
		Message: "存储配置更新成功",
	})
}
